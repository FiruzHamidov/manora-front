  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end gap-3">
        ...
        <Link href="/admin/new-buildings/stages/create">
          ...
          <tbody>
              ...
              <tr>
                <td className="px-4 py-6 text-center" colSpan={5}>
                  {search
                    ...
                    : 'Этапы строительства не найдены'}
                </td>
              </tr>
            )}
            ...
            {filteredItems.map((stage) => (
              <tr key={stage.id} className="border-t">
                <td className="px-4 py-3 text-gray-500">#{stage.id}</td>
                <td className="px-4 py-3 font-medium">{stage.name}</td>
                <td className="px-4 py-3 text-gray-500">{stage.slug}</td>
                <td className="px-4 py-3 text-gray-500">
                  {formatDate(stage.created_at)}
                </td>
                <td className="px-4 py-3">
                  <div className="flex justify-end gap-2">
                    <Link href={`/admin/new-buildings/stages/${stage.id}/edit`}>
