                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex justify-end gap-2">
                    <button
                      ...
                      onClick={() => onEdit(item)}
                      className="min-h-11 min-w-11 p-2 rounded-md hover:bg-gray-100 text-gray-600 hover:text-gray-900 transition"
                    >
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button
                      type="button"
                      aria-label={`Удалить запись №${item.id}`}
                      onClick={() => onDelete(item)}
                      className="min-h-11 min-w-11 p-2 rounded-md hover:bg-red-50 text-red-600 hover:text-red-700 transition"
                      disabled={deleting}
                    >
          ...
          <article key={item.id} className="rounded-2xl border bg-white p-4">
            {columns.map((column) => (
              <div key={`${item.id}-${column.label}`} className="flex justify-between gap-4 text-sm py-1">
                ...
                <span className="min-w-0 break-words text-right font-medium text-gray-900">{column.render(item)}</span>
              </div>
            ))}
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button
                type="button"
                onClick={() => onEdit(item)}
                className="min-h-11 px-3 py-2 rounded-md border hover:bg-gray-50"
              >
                Изменить
              </button>
              <button
                type="button"
                onClick={() => onDelete(item)}
                className="min-h-11 px-3 py-2 rounded-md border border-red-200 text-red-700 hover:bg-red-50"
                disabled={deleting}
              >
                Удалить
              </button>
