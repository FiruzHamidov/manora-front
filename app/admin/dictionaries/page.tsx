...
export default function AdminDictionariesPage() {
  const [tab, setTab] = useState<TabName>('Недвижимость');

  const propertyTypes = useDictionaryEntries('property-types');
  const propertyStatuses = useDictionaryEntries('property-statuses');
  const buildingTypes = useDictionaryEntries('building-types');
  const parkingTypes = useDictionaryEntries('parking-types');
  const heatingTypes = useDictionaryEntries('heating-types');
  const repairTypes = useDictionaryEntries('repair-types');
  const contractTypes = useDictionaryEntries('contract-types');
  const locations = useDictionaryEntries('locations');
  ...
  const carCategories = useDictionaryEntries('car-categories');
  const carBrands = useDictionaryEntries('car-brands');
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium ${
              tabName === tab
                ...
                : 'border border-[#D0D5DD] text-[#344054] bg-white'
            }`}
          >
            {tabName}
          </button>
