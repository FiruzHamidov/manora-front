...
export const ApplicationForm = ({id, title}: ApplicationFormProps) => {
  const { submitLead } = useLeadSubmission();
    ...
    // refs, чтобы сфокусироваться на первом ошибочном поле
    const refs = {
    ...
    const validate = (data: FormState): FormErrors => {
        ...
        // phone (простой формат: 7–15 цифр, допускаем +, пробелы, дефисы и скобки)
        ...
        // area (если заполнено — должно быть числом > 0)
        if (t(data.area)) {
            const num = Number(t(data.area).replace(',', '.'));
            if (Number.isNaN(num) || num <= 0) errs.area = 'Укажите площадь в м² (число > 0)';
            else if (num > 10000) errs.area = 'Слишком большая площадь';
        }

        // address (необязательное, но ограничим длину)
        if (t(data.address).length > 200) errs.address = 'Слишком длинный адрес';

        // message (необязательное, ограничим длину)
        if (t(data.message).length > 1000) errs.message = 'Слишком длинное сообщение';

        // rooms (необязательное)
    ...
    const focusFirstError = (errs: FormErrors) => {
        ...
        for (const key of order) {
            if (errs[key]) {
                const el =
                    key === 'message'
                        ? refs.message.current
                        : key === 'requestType'
                            ? refs.requestType.current
                            : key === 'phone'
                                ? refs.phone.current
                                : key === 'area'
                                    ? refs.area.current
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            const result = await submitLead({
                lead: {
                    service_type: title || 'Продажа недвижимости',
                    name: formData.name.trim(),
                    phone: formData.phone.trim(),
                    comment: formData.message?.trim() || undefined,
                    source: id || 'web-sell-property-form',
    ...
    // UI helpers
    const inputBase =
        'w-full h-[50px] md:h-[56px] pl-11 pr-3 bg-[#EEF2F6] rounded-xl border placeholder:text-[#8891A7] outline-none focus:outline-none focus:ring-0';
    const iconBase =
        'absolute inset-y-0 left-0 pl-3 flex items-center text-[#006341] pointer-events-none';
    const errText = 'mt-1 text-sm text-red-600';
    const withErr = (key: keyof FormState) =>
        `${inputBase} ${errors[key] ? 'border-red-500' : 'border-transparent'}`;

    return (
        <div className="bg-gradient-to-br from-[#006341] to-[#008A5A] rounded-3xl overflow-hidden" id={id}>
            <div className="px-5 md:px-20 py-7 md:py-[60px]">
                ...
                <div className="bg-white rounded-2xl p-4 md:p-6 lg:p-8">
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6" noValidate>
                        {/* Name */}
                        <div>
                            ...
                            <div className="relative">
                                <span className={iconBase}>
                                    ...
                                    >
                                        <circle cx="12" cy="6" r="4" fill="#006341"/>
                                        <ellipse cx="12" cy="17" rx="7" ry="4" fill="#006341"/>
                                      </svg>
                                </span>
                                <input
                                    ref={refs.name}
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <select
                                    ...
                                    onChange={handleInputChange}
                                    className={`${withErr('requestType')} appearance-none pr-9`}
                  ...
                  >
                    ...
                    />
                                ...
                                <input
                                    ...
                                    onChange={handleInputChange}
                                    placeholder="мин м²"
                                    className={withErr('area')}
                                    aria-invalid={!!errors.area}
                                    aria-describedby={errors.area ? 'err-area' : undefined}
                                />
                            </div>
                            {errors.area && <p id="err-area" className={errText}>{errors.area}</p>}
                        </div>
                  ...
                  >
                    ...
                    />
                                ...
                                <input
                                    ...
                                    className={withErr('address')}
                                    aria-invalid={!!errors.address}
                                    aria-describedby={errors.address ? 'err-address' : undefined}
                                />
                            </div>
                            {errors.address && <p id="err-address" className={errText}>{errors.address}</p>}
                        </div>
                        ...
                        <div>
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={
                                                ...
                                                (active
                                                    ? 'bg-white border-2 border-[#006341] text-[#006341]'
                            ...
                            {/* rooms необязательное — подсказка не нужна */}
                        </div>
                        ...
                        <div className="md:col-span-2">
                            ...
                            <textarea
                                ...
                                className={`w-full min-h-[120px] px-3 py-3 bg-[#EEF2F6] rounded-xl border placeholder:text-[#8891A7] outline-none focus:outline-none focus:ring-0 ${
                        ...
                        <div className="md:self-end">
                            <button
                                ...
                                className="w-full md:w-auto mt-2 md:mt-0 bg-[#006341] text-white px-5 h-[50px] rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {isSubmitting ? 'Отправка...' : 'Оценить недвижимость бесплатно'}
                            </button>
                        </div>
                    </form>
                </div>
